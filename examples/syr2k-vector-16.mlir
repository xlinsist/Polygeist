...
#map2 = affine_map<(d0) -> ((d0 + 1) floordiv 16)>
module attributes {llvm.data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128", llvm.target_triple = "x86_64-unknown-linux-gnu", "polygeist.target-cpu" = "x86-64", "polygeist.target-features" = "+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87", "polygeist.tune-cpu" = "generic"} {
  ...
  func.func private @kernel_syr2k(%arg0: i32, %arg1: i32, %arg2: f64, %arg3: f64, %arg4: memref<?x1200xf64>, %arg5: memref<?x1000xf64>, %arg6: memref<?x1000xf64>) attributes {llvm.linkage = #llvm.linkage<internal>} {
    ...
    %beta = vector.broadcast %arg3 : f64 to vector<16xf64>
    %alpha = vector.broadcast %arg2 : f64 to vector<16xf64>
    %mask_vec = arith.constant dense<1> : vector<16xi1>
    %stride_vec = arith.constant dense<[0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000, 13000, 14000, 15000]> : vector<16xindex>
    ...
    scf.for %i = %c0 to %1 step %c1 {
      ...
      scf.for %j_iter = %c0 to %j_iter_time step %c1 {
        ...
        %b_vec = vector.load %arg4[%i, %j0] : memref<?x1200xf64>, vector<16xf64>
        %result_vec = arith.mulf %b_vec, %beta : vector<16xf64>
        vector.store %result_vec, %arg4[%i, %j0] : memref<?x1200xf64>, vector<16xf64>
      }
      ...
      scf.if %tail_flag {
        ...
        %b_vec_tail = vector.maskedload %arg4[%i, %j0], %mask_tail_vec, %c0_f64_vec : memref<?x1200xf64>, vector<16xi1>, vector<16xf64> into vector<16xf64>
        %result_vec_tail = arith.mulf %b_vec_tail, %beta : vector<16xf64>
        vector.maskedstore %arg4[%i, %j0], %mask_tail_vec, %result_vec_tail : memref<?x1200xf64>, vector<16xi1>, vector<16xf64>
      }
      ...
      scf.for %k = %c0 to %0 step %c1 {
        ...
        scf.for %j_iter = %c0 to %j_iter_time step %c1 {
          ...
          %Cij_load = vector.load %arg4[%i, %j] : memref<?x1200xf64>, vector<16xf64>
          %Cij_store = arith.addf %Cij_load, %fma_vec : vector<16xf64>
          vector.store %Cij_store, %arg4[%i, %j] : memref<?x1200xf64>, vector<16xf64>
        }
        scf.if %tail_flag {
          %mask_tail_vec = vector.create_mask %tail_len : vector<16xi1>
          ...
          %mul3_vec = arith.mulf %alpha, %Bjk : vector<16xf64>
          %fma_vec = vector.fma %mul3_vec, %Aik, %mul2_vec : vector<16xf64>
          %Cij_load = vector.load %arg4[%i, %j0] : memref<?x1200xf64>, vector<16xf64>
          %Cij_store = arith.addf %Cij_load, %fma_vec : vector<16xf64>
          vector.store %Cij_store, %arg4[%i, %j0] : memref<?x1200xf64>, vector<16xf64>
        }
      }
    }
    return
