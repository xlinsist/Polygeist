...
#map2 = affine_map<(d0) -> ((d0 + 1) ceildiv 32)>
module attributes {llvm.data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128", llvm.target_triple = "x86_64-unknown-linux-gnu", "polygeist.target-cpu" = "x86-64", "polygeist.target-features" = "+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87", "polygeist.tune-cpu" = "generic"} {
  ...
  func.func private @kernel_syr2k(%arg0: i32, %arg1: i32, %arg2: f64, %arg3: f64, %arg4: memref<?x1200xf64>, %arg5: memref<?x1000xf64>, %arg6: memref<?x1000xf64>) attributes {llvm.linkage = #llvm.linkage<internal>} {
    ...
    %a_vec = vector.broadcast %arg3 : f64 to vector<32xf64>
    %alpha = vector.broadcast %arg2 : f64 to vector<32xf64>
    %mask_vec = arith.constant dense<1> : vector<32xi1>
    %stride_vec = arith.constant dense<[0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 0, 1000, 2000, 3000, 4000, 5000, 6000, 7000]> : vector<32xindex>
    %stride_mul_vec = arith.muli %stride_vec, %step_vec : vector<32xindex>
    ...
    affine.for %i = 0 to %1 {
      ...
      affine.for %j = 0 to #map2(%i) {
        ...
        scf.if %tail_flag {
          %b_vec = affine.vector_load %arg4[%i, %j] : memref<?x1200xf64>, vector<32xf64>
          %result_vec = arith.mulf %b_vec, %a_vec : vector<32xf64>
          affine.vector_store %result_vec, %arg4[%i, %j] : memref<?x1200xf64>, vector<32xf64>
        } else {
          %mask_tail_vec = vector.create_mask %tail_len : vector<32xi1>
          %b_vec_tail = vector.maskedload %arg4[%i, %j], %mask_tail_vec, %c0_f64_vec : memref<?x1200xf64>, vector<32xi1>, vector<32xf64> into vector<32xf64>
          %result_vec_tail = arith.mulf %b_vec_tail, %a_vec : vector<32xf64>
          vector.maskedstore %arg4[%i, %j], %mask_tail_vec, %result_vec_tail : memref<?x1200xf64>, vector<32xi1>, vector<32xf64>
        }
      }
      affine.for %k = 0 to %0 {
        ...
        affine.for %j = 0 to #map2(%i) {
          ...
          scf.if %tail_flag {
            ...
            %Cij_load = affine.vector_load %arg4[%i, %j] : memref<?x1200xf64>, vector<32xf64>
            %Cij_store = arith.addf %Cij_load, %fma_vec : vector<32xf64>
            affine.vector_store %Cij_store, %arg4[%i, %j] : memref<?x1200xf64>, vector<32xf64>
          } else {
            // tail processing
          }
        }
