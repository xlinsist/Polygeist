...
#map2 = affine_map<(d0) -> ((d0 + 1) floordiv 128)>
  ...
  func.func private @kernel_syr2k(%arg0: i32, %arg1: i32, %arg2: f64, %arg3: f64, %arg4: memref<?x1200xf64>, %arg5: memref<?x1000xf64>, %arg6: memref<?x1000xf64>) attributes {llvm.linkage = #llvm.linkage<internal>} {
    ...
    scf.for %i = %c0 to %1 step %c1 {
      ...
      scf.for %j_iter = %c0 to %j_iter_time step %c1 {
        ...
        %b_vec = vector.load %arg4[%i, %j0] : memref<?x1200xf64>, vector<128xf64>
        %result_vec = arith.mulf %b_vec, %beta : vector<128xf64>
        vector.store %result_vec, %arg4[%i, %j0] : memref<?x1200xf64>, vector<128xf64>
      }
      %j0 = arith.muli %j_iter_time, %step : index
      %tail_len = arith.subi %i_plus_1, %j0 : index
      %tail_flag = arith.cmpi sgt, %tail_len, %c0 : index
      scf.if %tail_flag {
        %mask_tail_vec = vector.create_mask %tail_len : vector<128xi1>
        %b_vec_tail = vector.maskedload %arg4[%i, %j0], %mask_tail_vec, %c0_f64_vec : memref<?x1200xf64>, vector<128xi1>, vector<128xf64> into vector<128xf64>
        %result_vec_tail = arith.mulf %b_vec_tail, %beta : vector<128xf64>
        vector.maskedstore %arg4[%i, %j0], %mask_tail_vec, %result_vec_tail : memref<?x1200xf64>, vector<128xi1>, vector<128xf64>
      }

      scf.for %k = %c0 to %0 step %c1 {
        %Aik_ele = memref.load %arg5[%i, %k] : memref<?x1000xf64>
        %Bik_ele = memref.load %arg6[%i, %k] : memref<?x1000xf64>
        %Aik = vector.broadcast %Aik_ele : f64 to vector<128xf64>
        %Bik = vector.broadcast %Bik_ele : f64 to vector<128xf64>

        scf.for %j_iter = %c0 to %j_iter_time step %c1 {
          %j = arith.muli %j_iter, %step : index
          %Ajk = vector.gather %arg5[%j, %k][%stride_vec], %mask_vec, %c0_f64_vec
            : memref<?x1000xf64>, vector<128xindex>, vector<128xi1>, vector<128xf64> into vector<128xf64>
          %Bjk = vector.gather %arg6[%j, %k][%stride_vec], %mask_vec, %c0_f64_vec
            : memref<?x1000xf64>, vector<128xindex>, vector<128xi1>, vector<128xf64> into vector<128xf64>
          %mul1_vec = arith.mulf %alpha, %Ajk : vector<128xf64>
          %mul2_vec = arith.mulf %mul1_vec, %Bik : vector<128xf64>
          %mul3_vec = arith.mulf %alpha, %Bjk : vector<128xf64>
          %fma_vec = vector.fma %mul3_vec, %Aik, %mul2_vec : vector<128xf64>
          %Cij_load = vector.load %arg4[%i, %j] : memref<?x1200xf64>, vector<128xf64>
          %Cij_store = arith.addf %Cij_load, %fma_vec : vector<128xf64>
          vector.store %Cij_store, %arg4[%i, %j] : memref<?x1200xf64>, vector<128xf64>
        }
        scf.if %tail_flag {
          %mask_tail_vec = vector.create_mask %tail_len : vector<128xi1>
          ...
          %mul3_vec = arith.mulf %alpha, %Bjk : vector<128xf64>
          %fma_vec = vector.fma %mul3_vec, %Aik, %mul2_vec : vector<128xf64>
          %Cij_load = vector.load %arg4[%i, %j0] : memref<?x1200xf64>, vector<128xf64>
          %Cij_store = arith.addf %Cij_load, %fma_vec : vector<128xf64>
          vector.store %Cij_store, %arg4[%i, %j0] : memref<?x1200xf64>, vector<128xf64>
        }
      }
    }
    return
