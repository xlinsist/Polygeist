...
#map2 = affine_map<(d0) -> ((d0 + 1) floordiv 64)>
module attributes {llvm.data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128", llvm.target_triple = "x86_64-unknown-linux-gnu", "polygeist.target-cpu" = "x86-64", "polygeist.target-features" = "+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87", "polygeist.tune-cpu" = "generic"} {
  ...
  func.func private @kernel_syr2k(%arg0: i32, %arg1: i32, %arg2: f64, %arg3: f64, %arg4: memref<?x1200xf64>, %arg5: memref<?x1000xf64>, %arg6: memref<?x1000xf64>) attributes {llvm.linkage = #llvm.linkage<internal>} {
    ...
    %c0_f64_vec = vector.splat %c0_f64 : vector<64xf64>
    %c1 = arith.constant 1 : index
    %step = arith.constant 64 : index
    %step_vec = arith.constant dense<1000> : vector<64xindex>
    %beta = vector.broadcast %arg3 : f64 to vector<64xf64>
    %alpha = vector.broadcast %arg2 : f64 to vector<64xf64>
    %mask_vec = arith.constant dense<1> : vector<64xi1>
    %stride_vec = arith.constant dense<[0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000, 13000, 14000, 15000, 16000, 17000, 18000, 19000, 20000, 21000, 22000, 23000, 24000, 25000, 26000, 27000, 28000, 29000, 30000, 31000, 32000, 33000, 34000, 35000, 36000, 37000, 38000, 39000, 40000, 41000, 42000, 43000, 44000, 45000, 46000, 47000, 48000, 49000, 50000, 51000, 52000, 53000, 54000, 55000, 56000, 57000, 58000, 59000, 60000, 61000, 62000, 63000]> : vector<64xindex>
    ...
    scf.for %i = %c0 to %1 step %c1 {
      ...
      scf.for %j_iter = %c0 to %j_iter_time step %c1 {
        ...
        %b_vec = vector.load %arg4[%i, %j0] : memref<?x1200xf64>, vector<64xf64>
        %result_vec = arith.mulf %b_vec, %beta : vector<64xf64>
        vector.store %result_vec, %arg4[%i, %j0] : memref<?x1200xf64>, vector<64xf64>
      }
      %j0 = arith.muli %j_iter_time, %step : index
      %tail_len = arith.subi %i_plus_1, %j0 : index
      %tail_flag = arith.cmpi sgt, %tail_len, %c0 : index
      scf.if %tail_flag {
        %mask_tail_vec = vector.create_mask %tail_len : vector<64xi1>
        %b_vec_tail = vector.maskedload %arg4[%i, %j0], %mask_tail_vec, %c0_f64_vec : memref<?x1200xf64>, vector<64xi1>, vector<64xf64> into vector<64xf64>
        %result_vec_tail = arith.mulf %b_vec_tail, %beta : vector<64xf64>
        vector.maskedstore %arg4[%i, %j0], %mask_tail_vec, %result_vec_tail : memref<?x1200xf64>, vector<64xi1>, vector<64xf64>
      }

      scf.for %k = %c0 to %0 step %c1 {
        %Aik_ele = memref.load %arg5[%i, %k] : memref<?x1000xf64>
        %Bik_ele = memref.load %arg6[%i, %k] : memref<?x1000xf64>
        %Aik = vector.broadcast %Aik_ele : f64 to vector<64xf64>
        %Bik = vector.broadcast %Bik_ele : f64 to vector<64xf64>

        scf.for %j_iter = %c0 to %j_iter_time step %c1 {
          %j = arith.muli %j_iter, %step : index
          %Ajk = vector.gather %arg5[%j, %k][%stride_vec], %mask_vec, %c0_f64_vec
            : memref<?x1000xf64>, vector<64xindex>, vector<64xi1>, vector<64xf64> into vector<64xf64>
          %Bjk = vector.gather %arg6[%j, %k][%stride_vec], %mask_vec, %c0_f64_vec
            : memref<?x1000xf64>, vector<64xindex>, vector<64xi1>, vector<64xf64> into vector<64xf64>
          %mul1_vec = arith.mulf %alpha, %Ajk : vector<64xf64>
          %mul2_vec = arith.mulf %mul1_vec, %Bik : vector<64xf64>
          %mul3_vec = arith.mulf %alpha, %Bjk : vector<64xf64>
          %fma_vec = vector.fma %mul3_vec, %Aik, %mul2_vec : vector<64xf64>
          %Cij_load = vector.load %arg4[%i, %j] : memref<?x1200xf64>, vector<64xf64>
          %Cij_store = arith.addf %Cij_load, %fma_vec : vector<64xf64>
          vector.store %Cij_store, %arg4[%i, %j] : memref<?x1200xf64>, vector<64xf64>
        }
        scf.if %tail_flag {
          %mask_tail_vec = vector.create_mask %tail_len : vector<64xi1>
          ...
          %mul3_vec = arith.mulf %alpha, %Bjk : vector<64xf64>
          %fma_vec = vector.fma %mul3_vec, %Aik, %mul2_vec : vector<64xf64>
          %Cij_load = vector.load %arg4[%i, %j0] : memref<?x1200xf64>, vector<64xf64>
          %Cij_store = arith.addf %Cij_load, %fma_vec : vector<64xf64>
          vector.store %Cij_store, %arg4[%i, %j0] : memref<?x1200xf64>, vector<64xf64>
        }
      }
    }
    return
